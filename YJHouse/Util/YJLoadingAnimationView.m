...
@implementation YJLoadingAnimationView {
...
-(void)buildUI {
    ...
    for (NSInteger i = 0; i<textColors.count; i++) {
        ...
        YJLoadingView *dot = [[YJLoadingView alloc] initWithFrame:CGRectMake(dotX, 0, [self dotWidth],[self dotWidth])];
...
-(void)reloadView
{
    YJLoadingView *dot1 = _dots.firstObject;
    
    YJLoadingView *dot2 = _dots.lastObject;
...
//显示放大、缩小动画
- (void)showAnimationsOfDot:(YJLoadingView *)dot {
...
+ (YJLoadingAnimationView *)getLoadingInView:(UIView *)view {
    YJLoadingAnimationView *loading = nil;
    for (YJLoadingAnimationView *subview in view.subviews) {
        if ([subview isKindOfClass:[YJLoadingAnimationView class]]) {
            loading = subview;
        }
    }
    return loading;
...
+ (void)showInView:(UIView*)view frame:(CGRect)frame{
    YJLoadingAnimationView *loading = [[YJLoadingAnimationView alloc] initWithFrame:frame];
    [view addSubview:loading];
    [loading start];
}

+ (void)hideInView:(UIView *)view {
    YJLoadingAnimationView *loading = [YJLoadingAnimationView getLoadingInView:view];
    if (loading) {
