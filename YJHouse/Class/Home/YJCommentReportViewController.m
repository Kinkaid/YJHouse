...
- (void)confirmSend:(UIButton*)sender {
    NSDictionary *params = @{@"site":self.site_id,@"tid":self.house_id,@"auth_key":[LJKHelper getAuth_key],@"user_id":[LJKHelper getUserID],@"type":@"2",@"id":self.commentID,@"content_id":[self.commentTV.text hasPrefix:@"详细描述..."] ? @"":self.commentTV.text};
